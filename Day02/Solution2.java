...
public class Solution2 {
    ...
    private static int isSafe(List<Integer> report, boolean firstRound){
        ...
        for(int level = 0 ; level < report.size() - 1 ; level++){
            ...
            if(Math.abs(report.get(level+1) - report.get(level)) > 3) return dampenProblem(report, level, 2, firstRound);
            if(report.get(level+1) > report.get(level) && !isAsc) return dampenProblem(report, level, 1, firstRound);
            if(report.get(level+1) < report.get(level) && isAsc) return dampenProblem(report, level, 1, firstRound);
        }
        return 1;
    }
